...
#ifndef FASTPSI_UINT_H_
#define FASTPSI_UINT_H_
...
class uint {
 public:
  ...
  constexpr uint(__uint128_t v0) { values_[0] = v0; }
  ...
  constexpr uint(__uint128_t v0, __uint128_t v1, __uint128_t v2, __uint128_t v3,
                 __uint128_t v4) {
    values_[0] = v0;
    values_[1] = v1;
    values_[2] = v2;
  ...
  constexpr explicit operator int() const {
    return static_cast<int>(values_[0]);
  }
  ...
  friend constexpr __uint128_t Uint(uint<Size> v, int index) {
    return v.values_[index];
  }
...
constexpr inline uint<1> MakeUint(__uint128_t v0) { return uint<1>(v0); }
...
constexpr inline uint<2> operator|(uint<2> lhs, uint<2> rhs) {
  return MakeUint(Uint(lhs, 0) | Uint(rhs, 0), Uint(lhs, 1) | Uint(rhs, 1));
}
...
constexpr inline uint<4> operator|(uint<4> lhs, uint<4> rhs) {
  return MakeUint(Uint(lhs, 0) | Uint(rhs, 0), Uint(lhs, 1) | Uint(rhs, 1),
                  Uint(lhs, 2) | Uint(rhs, 2), Uint(lhs, 3) | Uint(rhs, 3));
}
...
constexpr inline uint<6> operator|(uint<6> lhs, uint<6> rhs) {
  return MakeUint(Uint(lhs, 0) | Uint(rhs, 0), Uint(lhs, 1) | Uint(rhs, 1),
                  Uint(lhs, 2) | Uint(rhs, 2), Uint(lhs, 3) | Uint(rhs, 3),
                  Uint(lhs, 4) | Uint(rhs, 4), Uint(lhs, 5) | Uint(rhs, 5));
}
...
constexpr inline uint<2> operator>>(uint<2> lhs, int amount) {
  // Only for amount < 128
  return MakeUint(Uint(lhs, 0) >> amount | (Uint(lhs, 1) << (128 - amount)),
                  Uint(lhs, 1) >> amount);
}
...
constexpr inline bool operator==(uint<Size> lhs, int rhs) {
  bool equals = static_cast<int>(Uint(lhs, 0)) == rhs;
...
template <int Size>
constexpr inline bool operator!=(uint<Size> lhs, int rhs) {
  bool not_equals = Uint(lhs, 0) != static_cast<__uint128_t>(rhs);
  for (int i = 1; i < Size; i++) {
